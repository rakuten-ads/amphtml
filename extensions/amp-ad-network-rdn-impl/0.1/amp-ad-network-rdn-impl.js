...
/**
 * RDN endpoint for AMP
 *
 * @type {string}
 * @private
 */
const RDN_A4A_ENDPOINT = 'https://s-ad.rmp.rakuten.co.jp/amp';

/** @const {string} */
const TAG = 'amp-ad-network-rdn-impl';

export class AmpAdNetworkRDNImpl extends AmpA4A {
  /**
  ...
  getAdUrl() {
    ...
    const ep = this.element.getAttribute('src') || RDN_A4A_ENDPOINT;
  ...
  extractAdInfo(responseHeaders) {
    this.trackerConfig_ = tryParseJson(
      responseHeaders.get('X-Rdn-Tracker') || {}
    );
  ...
  isValidElement() {
    if (!this.element.getAttribute('data-id')) {
      user().warn(TAG, 'RDN ads require the attribute data-id');
      return false;
    }
    return true;
  }
...
AMP.extension('amp-ad-network-rdn-impl', '0.1', AMP => {
  AMP.registerElement('amp-ad-network-rdn-impl', AmpAdNetworkRDNImpl);
});
